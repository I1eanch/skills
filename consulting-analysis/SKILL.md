...
description: Консалтинговые аналитические отчёты уровня McKinsey/BCG — анализ рынка, потребительские инсайты, бренд-стратегия, финансовый анализ, отраслевое исследование, конкурентная разведка, инвестиционная оценка. Работает в две фазы: (1) каркас анализа — скелет глав, выбор аналитических моделей, требования к данным с готовыми поисковыми запросами и план визуализации; (2) после сбора данных другими скиллами — финальный отчёт со связным нарративом, графиками и стратегическими выводами. Использовать при запросах «проанализируй рынок», «конкурентный анализ», «стратегия продукта», «оценить нишу», «исследование отрасли», «юнит-экономика и воронка», «due diligence». Triggers on market analysis, consumer insights, competitive intelligence, industry research, investment due diligence, consulting-grade report.
---
...
The output adheres to McKinsey/BCG consulting voice standards. The report language follows the `output_locale` setting (default: `ru_RU` — Russian).
...
- Generate inline citations as markdown links and a reference list formatted per **ГОСТ Р 7.0.5-2008**
...
After the analysis framework is generated, it is handed off to **other skills of this instance** for data collection:
...
5. **References** — reference list per **ГОСТ Р 7.0.5-2008**
...
3. **Delegate to Visualization Skills**: first `chart-visualization` — confirm the chart type fits the data and read `references/generate_<type>.md` for the exact required fields; then the renderer (`visual-explainer`, `recharts` or `ckm-slides`) with:
   ...
   - Output file path convention: `charts/chapter_{N}_{chart_index}.html` (or `.svg`); use `.png` only if the chosen skill actually rasterizes
...
- **Number Formatting**: For Russian output use a non-breaking space as the thousands separator (`1 000`) and a comma as the decimal separator (`12,5 %`); for English output use `1,000` and `12.5%`
...
- **References section**: Formatted strictly per **ГОСТ Р 7.0.5-2008** — `Автор. Заглавие [Электронный ресурс]. URL: <ссылка> (дата обращения: ДД.ММ.ГГГГ).`
...
9. 7. References — ГОСТ Р 7.0.5-2008 formatted list
...
- [ ] Numbers follow the locale rules (`1 000` / `12,5 %` for Russian)
- [ ] Inline references use markdown links where applicable
- [ ] References section follows ГОСТ Р 7.0.5-2008
...
```
output_locale = ru_RU  # configurable per user request
reasoning_locale = en
```
...
## Хранение результата в vault

Каркас анализа и итоговый отчёт — долгоживущие материалы, поэтому они сохраняются по правилам vault, а не только выводятся в чат:

- **Каркас (фаза 1)** — `Plans/YYYY-MM-DD Каркас анализа <тема>.md`. Он является техзаданием на сбор данных, и к нему возвращаются между фазами.
- **Итоговый отчёт (фаза 2)** — `База знаний/<категория>/YYYY-MM-DD <Заголовок отчёта>.md` с frontmatter (`title`, `status: обработано`, `tags`, `date`), после чего добавляется запись в `База знаний/_Поисковый_индекс.md`.
- **Графики и выгрузки** — `_Resources/`; в отчёте на них ссылаются относительными путями.
- **Отчёт по проекту** — если тема относится к активному проекту, ссылка на отчёт добавляется в раздел «Связанные заметки» файла проекта в `Проекты/`, а принятые по итогам решения оформляются по `Проекты/_Шаблон Decision Log.md`.

Отчёт по разовому вопросу без долгой ценности остаётся в `Ежедневник/YYYY-MM-DD.md` и evergreen-заметкой не оформляется.

## Источник

Адаптировано из `bytedance/deer-flow`, `skills/public/consulting-analysis` (MIT). Методологический корпус — 28 аналитических моделей, принципы отбора, протокол достоверности данных, цепочка «Data → User Psychology → Strategy Implication», ограничения тона и оба чек-листа — сохранён без изменений.

Изменения адаптации:

- `output_locale` переведён с `zh_CN` на `ru_RU`.
- Китайский стандарт библиографических ссылок GB/T 7714-2015 заменён на ГОСТ Р 7.0.5-2008 (4 места).
- Правило разделителей чисел переписано под русскую типографику (`1 000`, `12,5 %`) вместо борьбы с китайской пунктуацией.
- Ссылки на несуществующие в этом инстансе скиллы заменены на реальные: сбор данных — `research`, `hyperresearch`, `data-analysis`, `github-deep-research`; графики — `visual-explainer`, `recharts`, `ckm-slides`.
- Соглашение об именах файлов графиков изменено с `.png` на `.html`/`.svg`, поскольку растеризация в этом инстансе не гарантирована.
- Добавлены раздел хранения результата в vault и русские триггеры в `description`.

...
description: Рецензия на научную статью уровня peer review для топовых конференций и журналов — краткое изложение, сильные и слабые стороны, оценка методологии и вклада, позиционирование в литературе, конкретные рекомендации, всё со опорой на текст самой работы. Использовать при запросах «разбери статью», «отрецензируй работу», «оцени эту публикацию», «что не так с этим исследованием», «сделай peer review», при ссылке на arXiv или приложенном PDF статьи. Triggers on paper URLs, arXiv links, PDF preprints, «review this paper», «analyze this research», «write a peer review». Для обзора многих статей по теме нужен систематический обзор литературы, а не рецензия на одну работу.
---
...
- Save the review to the vault as `База знаний/Исследования/YYYY-MM-DD Рецензия — <краткое название работы>.md` with frontmatter (`title`, `status: обработано`, `tags`, `date`), then add an entry to `База знаний/_Поисковый_индекс.md`. A throwaway review goes to `Ежедневник/YYYY-MM-DD.md` instead.
- State the saved path in the chat message — there is no separate file-presentation tool in this instance.
- Write the review in the user's language; the paper title, author names and quoted fragments stay in the original language.

## Notes

- This skill complements `research` and `hyperresearch` — load one of them alongside when the user wants the paper reviewed in the context of the broader field, and `systematic-literature-review` when the field itself needs a survey
...
Адаптировано из `bytedance/deer-flow`, `skills/public/academic-paper-review` (MIT). Методология рецензирования сохранена без изменений. Изменения: путь сохранения `/mnt/user-data/outputs/` заменён на vault, убрана отсылка к инструменту `present_files`, добавлены правило языка рецензии, русские триггеры в `description` и указание на парный скилл `systematic-literature-review` для обзора многих работ.
